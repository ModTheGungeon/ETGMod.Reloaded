    ...
    public static class GridPacker {
        ...
        public struct Element {
            ...
            public void DrawOn(Texture2D target, int xoffs, int yoffs) {
                ...
                int w = x2 - x1;
                int h = y2 - y1;
                _logger.Info($"Drawing xoff={xoffs}, yoff={yoffs}, On ({x1},{y1}) ({x2},{y2})");
                ...
                if (
                    ...
                    // drawing borders for debug
                    //for (int y = 0; y < h; y++)
                    //{
                    //    target.SetPixel(xoffs -1, target.height - h - yoffs + y, new Color(.3f, .8f, .3f));
                    //    target.SetPixel(xoffs + w , target.height - h - yoffs + y, new Color(.3f, .8f, .3f));
                    //}
                    //for (int x = 0; x < w; x++)
                    //{
                    //    target.SetPixel(xoffs + x, target.height - h - yoffs - 1, new Color(.3f, .8f, .3f));
                    //    target.SetPixel(xoffs + x, target.height - h - yoffs + h, new Color(.3f, .8f, .3f));
                    //}
                } else {
                    // flipped
                    if (UVs[0].x == UVs[1].x) {
                    ...
                    for (int y = 0; y < h; y++) {
                        ...
                        for (int x = 0; x < w; x++) {
                            ...
                            //if (x == 0)
                            //{
                            //    target.SetPixel(xoffs + x-1, target.height - h - yoffs + y, new Color(.3f, .8f, .3f));
                            //}else if(x == w - 1){
                            //    target.SetPixel(xoffs + x+1, target.height - h - yoffs + y, new Color(.3f, .8f, .3f));
                            //}else if( y == 0){
                            //    target.SetPixel(xoffs + x, target.height - h - yoffs + y-1, new Color(.3f, .8f, .3f));
                            //}else if(y == h - 1){
                            //    target.SetPixel(xoffs + x, target.height - h - yoffs + y+1, new Color(.3f, .8f, .3f));
                            //}else
                            //{
                ...
                string single_sprite_dir = Path.Combine(Paths.ResourcesFolder,"test_output");
                if (!Directory.Exists(single_sprite_dir))
                    Directory.CreateDirectory(single_sprite_dir);
                _logger.Info($"Dumping seperated sprite def $[{Name}]");
                File.WriteAllBytes(Path.Combine(single_sprite_dir, $"{Name}.png"), texRegion.EncodeToPNG());
            }
        }
